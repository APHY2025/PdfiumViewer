...
namespace CustomPdfiumViewer.Demo
{
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            _zoom.Text = pdfViewer.Renderer.Zoom.ToString();
        ...
        {
            _page.Text = (pdfViewer.Renderer.Page + 1).ToString();
        }
        ...
        private void btnZoomBottomRight_Click(object sender, EventArgs e)
        {
            pdfViewer.SetZoomWithAnchor(2.0f, ZoomAnchor.BottomRight);
            if (int.TryParse(_page.Text, out int pageNumber))
            {
                int zeroBasedPage = pageNumber - 1;
                if (zeroBasedPage >= 0 && zeroBasedPage < pdfViewer.Document.PageCount)
                {
                    pdfViewer.Renderer.Page = zeroBasedPage;
                }
                else
                {
                    MessageBox.Show("ページ番号が範囲外です");
                }
            }
            else
            {
                MessageBox.Show("ページ番号は数値で入力してください");
            }
        }
    }
}
