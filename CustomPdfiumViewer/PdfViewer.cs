...
namespace CustomPdfiumViewer
{
    ...
    public partial class PdfViewer : UserControl
    {
        ...
        public void SetZoomWithAnchor(float zoomPercent, ZoomAnchor anchor)
        {
            // fork版では ZoomMode, Zoom が無いので ScaleFactor を直接設定
            var renderer = Renderer;
            if (renderer == null) return;

            renderer.ScaleFactor = zoomPercent;
        ...
        public void ScrollToAnchorPosition(ZoomAnchor anchor)
        {
            var renderer = Renderer;
            if (renderer == null) return;

            // 表示される全ドキュメント範囲
            var docWidth = renderer.DisplayRectangle.Width;
            var docHeight = renderer.DisplayRectangle.Height;

            var clientWidth = renderer.ClientSize.Width;
            var clientHeight = renderer.ClientSize.Height;
            ...
            switch (anchor)
            {
                case ZoomAnchor.TopLeft:
                    x = 0; y = 0;
                    break;
                case ZoomAnchor.TopRight:
                    x = docWidth - clientWidth; y = 0;
                    break;
                case ZoomAnchor.BottomLeft:
                    x = 0; y = docHeight - clientHeight;
                    break;
                case ZoomAnchor.BottomRight:
                    x = docWidth - clientWidth; y = docHeight - clientHeight;
                    break;
            }

            // SetDisplayRectLocation の第1引数は "ドキュメントが移動すべき先" なので、負数で渡す
            renderer.SetDisplayRectLocation(new Point(-x, -y));
        }
    }
}
